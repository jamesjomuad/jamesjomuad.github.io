...
//
	APP.service('Service', function($filter) {
		...
		var expenses	= expenses ? expenses : 0;
...
//
	APP.controller("home", function($scope, Service) {
		...
		$scope.income	= Service.storage[Service.month];
	});
	...
	APP.controller("expenses", function($scope, $http, $filter,Service) {
		...
		$scope.Vars.currentMonth = Service.date.month;

		
		$scope.set = function(){
			if($scope.memory[$scope.Vars.currentMonth]!==undefined)
				$scope.expenses = $scope.memory[$scope.Vars.currentMonth];
			else
				$scope.memory[$scope.Vars.currentMonth] = {income:income,expenses:[]};
			
			//Set Expenses
			$scope.expenses = $scope.memory[$scope.Vars.currentMonth].expenses;
			
			//Set Income
			$scope.memory[$scope.Vars.currentMonth].income = $scope.income;
